...
from plotting import plotEphemerides
...
def constellationAnalysis(config, stepSize, points):
    ...
    initialDate = AbsoluteDate(2021, 1, 1, 23, 30, 0.000, TimeScalesFactory.getUTC())
    ...
    for x in range(constellations.t):
        satellites = satellite(constellations.initialOrbit[x], Moon, mass, stepSize)
    ...
    t = [initialDate.shiftedBy(float(dt)) for dt in arange(0, duration, stepSize)]

    y = 1

    if y == 0:
        plotEphemerides(propagators, t, Moon, points)
